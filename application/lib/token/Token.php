...
class Token
{
    public static function getToken($user,$isApp=false)
    {
        if(!$isApp){
            $accessToken = self::createAccessToken($user);
            $refreshToken = self::createRefreshToken($user);
        }else{
            $accessToken = self::createAppAccessToken($user);
            $refreshToken = self::createAppRefreshToken($user);
        }
     ...
     * @throws TokenException
     */
    public static function getCurrentWxOpenID($appID=1)
    {
        if($appID == ApplicationEnum::FU_JIA){
            $openid = self::getCurrentTokenVar('minipro_openid','app_access_token_salt');
        }else{
             throw new Exception();
        }
        return $openid;
     ...
     * @throws TokenException
     */
    public static function getCurrentMID($appID=1)
    {
        if($appID == ApplicationEnum::FU_JIA){
            $mid = self::getCurrentTokenVar('id','app_access_token_salt');
        }else{
            throw new Exception();
        }
        return $mid;
     ...
     * @throws TokenException
     */
    public static function refreshAppToken($appID = 1)
    {
        try {
            if($appID == ApplicationEnum::FU_JIA){
                $mid = self::getCurrentTokenVar('id','app_refresh_token_salt');
            }else{
                $mid =   self::getCurrentTokenVar('id','st_app_refresh_token_salt');
            }
            $member =  model('member')::get($mid);
            $accessToken = self::createAppAccessToken($member);
        } catch (TokenException $ex) {
            throw new TokenException(['msg' => $ex->msg,'error_code' => 10010]);
        }
    ...
    private static function createAppAccessToken($user)
    {
        if($user->application_id == ApplicationEnum::FU_JIA){
            $key = config('secure.app_access_token_salt');
        }else{
            $key = config('secure.st_app_access_token_salt');
        }
        $payload = [
            'iss' => 'yuxiangkeji-zero', //签发者
            'iat' => time(), //什么时候签发的
    ...
    private static function createAppRefreshToken($user)
    {
        if($user->application_id == ApplicationEnum::FU_JIA){
            $key = config('secure.app_refresh_token_salt');
        }else{
            throw new Exception();
        }
        $payload = [
